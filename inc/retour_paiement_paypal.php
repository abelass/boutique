...
function inc_retour_paiement_paypal_dist($token){
	...
	spip_log("NOTIFICATION paypal retour toke=$token","boutique_paypal");
	...
	if (!$fp) {
		spip_log("NOTIFICATION paypa erreur verification serveur","boutique");
		}
 	else {
		fputs ($fp, $header . $req);
		while (!feof($fp)) {
			...
			spip_log("NOTIFICATION paypal verify:$res","boutique");
			
			if (strcmp ($res, "VERIFIED") == 0) {
				// check the payment_status is Completed
				spip_log("NOTIFICATION paypal verification serveur ok","boutique");
				$contexte = Array();
				...
				$cont=sql_fetsel('*','spip_boutique_commande',Array("token = '".$contexte["token"]."'"));
				$cont['titre_produit']=titre_article($cont['id_produit']);
				$cont['constellation']=titre_mot($cont['id_produit']);	
				
				spip_log("NOTIFICATION  PAYPAL pour ".$contexte["token"]." avec le statut_paiement ".$contexte["payment_status"],"boutique");
				
				$les_entetes_get = print_r($_GET,true);
				$les_entetes_post = print_r($_POST,true);
				
				spip_log($les_entetes_get,"boutique_paypal");
				spip_log($les_entetes_post,"boutique_paypal");
				
				$token_client = sql_fetsel("token","spip_boutique_commande","token = '".$token."'");
				...
				* */
				...
				//variables par défaut pour l'envoi des mails
					...
					$message_paiement_client = _T('boutique:message_paiement_client');
					$message_paiement_webmaster = _T('boutique:message_paiement_webmaster');
					$message_validation= _T('boutique:message_validation_inconnue');
					$byby = '<p>'._T('boutique:byby').'</p><p>'.$GLOBALS['meta']['nom_site'].'</p>';				
					$footer= '</body></html>';	
				...
				// enregistrement dans la bd, les log et définition des variables mail selon les réponses de paypal
					...
					case 'Canceled-Reversal' :
						spip_log("NOTIFICATION Annulation d'un panier","boutique");
						
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
						
						$message_validation= _T('boutique:annule');			
					...
					case 'Completed' :
						...
						if(sql_fetsel('id_paiement','spip_boutique_tokens_retour',Array("id_paiement = '".$contexte["id_paiement"]."'"))) $erreur['id_paiment']=1;

						// check that receiver_email is your Primary PayPal email
						if($contexte["email_paypal"] != lire_config('boutique/email_paypal')) $erreur['email_paypal']=1;

						// check that payment_amount/payment_currency are correct
						$verifier=sql_fetsel('code_devise,prix,frais_livraison','spip_boutique_commande',Array("token = '".$contexte["token"]."'")); 
						...
						if(count($erreur)==0){					
							// process payment
							spip_log("NOTIFICATION Validation de paiement d'un panier","boutique");
							sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"paye"),Array("token = '".$contexte["token"]."'"));
							
							sql_updateq("spip_boutique_tokens_retour",Array('id_paiement'=>$contexte["id_paiement"]),Array("token_panier = '".$contexte["token"]."'"));	
													
							$message_paiement_client= _T('boutique:message_validation_accepte_client');
							$message_paiement_webmaster= _T('boutique:message_validation_accepte_webmaster');
							$message_validation= '';						
							...
							spip_log("ALERTE Validation du paiement echou&eacute; erreurs:".$details_erreur,"boutique");
					...
					case 'Denied' :
						spip_log("NOTIFICATION Refu de paiement d'un panier","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:rejete');	
					...
					case 'Expired' :
						spip_log("NOTIFICATION Delais de paiement d'un panier expire","boutique");
						ql_updateq("spip_boutique_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:expire');	
					...
					case 'Failed' :
						spip_log("NOTIFICATION Le paiement d'un panier a echoue","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:echoue');
					...
					case 'In-Progress' :
						spip_log("NOTIFICATION Un paiement d'un panier est en cour","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"pendant"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:pendant');
					...
					case 'Part-Ref' :
						spip_log("NOTIFICATION Un remboursement partiel d'un panier est en cour","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"rembourse_partiellement"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:rembourse_partiellement');
					...
					case 'Pending' :
						spip_log("NOTIFICATION Paiement d'un panier en attente chez paypal. Cause: "._request('pending_reason'),"boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"attente"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:attente');
					...
					case 'Refunded' :
						spip_log("NOTIFICATION Un paiement d'un panier est rembourse","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"rembourse"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:rembourse');
					...
					case 'Reversed' :
						spip_log("NOTIFICATION Le paiement d'un panier a echoue pour les raisons suivantes: "._request('ReasonCode'),"boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"annule"),Array("token = '".$contexte["token"]."'"));
						$message_validation= _T('boutique:rejete');
					...
					$erreur=array();
						...
						if(count($erreur)==0){	
							spip_log("NOTIFICATION Paiement d'un panier accepte","boutique");
							sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"paye"),Array("token = '".$contexte["token"]."'"));
							$message_paiement_client= _T('boutique:message_validation_accepte_client');
							$message_paiement_webmaster= _T('boutique:message_validation_accepte_webmaster');
							$message_validation= '';
					...
					case 'Voided' :
						spip_log("NOTIFICATION Paiement d'un panier accepte","boutique");
						sql_updateq("spip_boutique_commande",Array("statut_paiement"=>"1_Voided"),Array("token = '".$contexte["token"]."'"));
						/*notifier_client($token_client['token_client'], _T("paypal:votre_paiement_a_bien_ete_valide_depuis_le_site_de_paypal"));
						notifier_marchand(_T("paypal:un_paiement_a_bien_ete_valide_depuis_le_site_de_paypal")."\n Voir le panier: ".generer_url_ecrire('echoppe_edit_panier','token='.$contexte["token"],"&"));
					...
					default :
						...
						spip_log("NOTIFICATION  PAYPAL pour ".$contexte["token"]." sans statut_paiement !","boutique");	
		...
		spip_log("NOTIFICATION  PAYPAL not valid pour","boutique");	
			}
		}
	fclose ($fp);
	}
