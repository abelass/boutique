...
function inc_retour_paiement_paypal_dist($token){
	...
	spip_log("NOTIFICATION  PAYPAL pour ".$contexte["token"]." avec le statut ".$contexte["payment_status"],"boutique");
	...
	switch ($contexte["payment_status"]) {
		...
		case 'Canceled-Reversal' :
			...
			sql_updateq("spip_boutique_commande",Array("statut"=>"annule"),Array("token = '".$contexte["token"]."'"));
...
// 			echoppe_informer_panier($contexte["token"],"Canceled-Reversal","Une annulation de paiement a été annulée. Par exemple, vous avez obtenu gain de cause dans un litige avec le client et les fonds de la transaction qui avaient été remboursés vous sont retournés.","paypal",0,"warning");
		...
		case 'Completed' :
			...
			sql_updateq("spip_boutique_commande",Array("statut"=>"paye"),Array("token = '".$contexte["token"]."'"));
...
// 		break;
		...
		case 'Expired' :
			...
			ql_updateq("spip_boutique_commande",Array("statut"=>"annule"),Array("token = '".$contexte["token"]."'"));
		...
		case 'In-Progress' :
			...
			sql_updateq("spip_boutique_commande",Array("statut"=>"1_progress"),Array("token = '".$contexte["token"]."'"));
		...
		case 'Part-Ref' :
			...
			sql_updateq("spip_boutique_commande",Array("statut"=>"1_Part-Ref"),Array("token = '".$contexte["token"]."'"));
		...
		case 'Pending' :
			spip_log("NOTIFICATION Paiement d'un panier en attente chez paypal. Cause: "._request('pending_'),"boutique");
		...
		case 'Refunded' :
			...
			sql_updateq("spip_boutique_commande",Array("statut"=>"1_Refunded"),Array("token = '".$contexte["token"]."'"));
		...
		case 'Voided' :
			spip_log("NOTIFICATION Paiement d'un panier accepte","boutique");
			sql_updateq("spip_boutique_commande",Array("statut"=>"1_Voided"),Array("token = '".$contexte["token"]."'"));
			/*notifier_client($token_client['token_client'], _T("paypal:votre_paiement_a_bien_ete_valide_depuis_le_site_de_paypal"));
			notifier_marchand(_T("paypal:un_paiement_a_bien_ete_valide_depuis_le_site_de_paypal")."\n Voir le panier: ".generer_url_ecrire('echoppe_edit_panier','token='.$contexte["token"],"&"));
		...
		default :
			...
			spip_log("NOTIFICATION  PAYPAL pour ".$contexte["token"]." sans statut !","boutique");	
